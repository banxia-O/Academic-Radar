...
一个**给临床医生 / 医学方向科研人员 / 研究生**的学术信息雷达插件。每天定时从 PubMed、Semantic Scholar、OpenAlex、bioRxiv、medRxiv、X（推特）、RSS 抓取你研究方向相关的论文，LLM 打分筛选去重后，推送到你的微信、Telegram、Slack 或邮箱。
...
| 渠道 | 你需要拿到 |
|------|-----------|
| **微信群机器人**（最方便） | 在企业微信/钉钉群"添加机器人 → 自定义机器人"，复制 webhook URL |
...
> 如果你用的 Agent **自己有邮箱**（某些云端集成版本会给 Agent 配独立邮箱），可以让 Agent 用自己的邮箱接收，再转给你。不过最省事的还是微信群机器人——手机直接收，免费，不用切应用。
...
```
6 个数据源 → 关键词初筛（脚本层，省 token）→ LLM 1-5 分相关性评分
  → DOI 验证 + 发表状态识别 → 跨源去重合并 → 排序 → 推送 + 存档
```
...
**容错**：单个数据源挂了不阻塞其他源，最多某次少几条。漏跑（机器关机）会自动补抓最近 48 小时内的内容，不会断档。
...
| Python | ≥ 3.9 |
| 一台常开的机器 | 跑定时任务用，本机 / VPS / 云主机都行。笔记本经常关机也没事——下次开机会自动补抓 48 小时内的内容 |
